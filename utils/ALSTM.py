...
class ALSTM(nn.Module):
    ...
    def __init__(self, input_num, hidden_num, num_layers,out_num,feature=False):
        ...
        for i in range(0, self.num_layers):
            ...
            cell_list.append(nn.LSTMCell(cur_input_num,self.hidden_num[i]))
        ...
        for t in range(seq_len):
            ...
            for layer_idx in range(self.num_layers):
                ...
                if 0==layer_idx:
                    ...
                    attentionmaps.append(attention_map.view(attention_map.size(0),7,7))
            ...
            hct_1=output_t
            out_list.append(output_t[-1][0])
        self.attention_maps = attentionmaps
#         return output_t[-1][0] # shape: (batch, hidden_layer[-1]=256)
        if self.feature:
            return output_t[-1][0] # shape: (batch, seq_len, hidden_layer[-1]=256)
#             return torch.stack(out_list,1) # shape: (batch, seq_len, hidden_layer[-1]=256)
        return self.fc(output_t[-1][0]) # shape: (batch, hidden_layer[-1]=256)

    def _init_hidden(self, batch_size):
        init_states = []
        for i in range(self.num_layers):
            init_states.append([torch.zeros(batch_size, self.hidden_num[i]).cuda(),torch.zeros(batch_size, self.hidden_num[i]).cuda()])
        return init_states
